<?php 
	...
	$no = 1;
	$sql = $connect->query("SELECT * FROM kriteria");
	...
	$pdf->SetX(8); 
	$pdf->Cell(0,0.3,'Data Kriteria',0,1);
	...
	$pdf->Cell(1,0.5,'No',1);
	//$pdf->Cell(1,0.5,'ID',1);
	$pdf->Cell(4,0.5,'Nama Kriteria',1);
	$pdf->Cell(4,0.5,'Kaidah',1);
	$pdf->Cell(2,0.5,'Bobot',1);
	$pdf->Cell(3,0.5,'Tipe Preferensi',1);
	$pdf->Cell(1.5,0.5,'p',1);
	$pdf->Cell(2,0.5,'q',1);
	...
	$pdf->Cell(4,0.5,''.$data['nama_kriteria'].'',1);
	$pdf->SetFont('Courier','B',7);
	$pdf->Cell(4,0.5,''.$data['kaidah'].'',1);
	$pdf->SetFont('Courier','B',7);
	$pdf->Cell(2,0.5,''.$data['bobot'].'',1);
	$pdf->SetFont('Courier','B',8);
	$pdf->Cell(3,0.5,''.$data['tipe_preferensi'].'',1);
	$pdf->Cell(1.5,0.5,''.$data['p'].'',1);
	$pdf->Cell(2,0.5,''.$data['q'].'',1);
	$pdf->Ln(0.5);
	}
