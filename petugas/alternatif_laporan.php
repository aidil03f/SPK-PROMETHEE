<?php 
	...
	$no = 1;
	$sql = $connect->query("SELECT * FROM alternatif_supplier");
	...
	$pdf->SetX(8); 
	$pdf->Cell(0,0.3,'Data Supplier',0,1);
	...
	$pdf->Cell(1,0.5,'No',1);
	//$pdf->Cell(1,0.5,'ID',1);
	$pdf->Cell(3,0.5,'Nama Supplier',1);
	$pdf->Cell(3,0.5,'Phone',1);
	$pdf->Cell(10.5,0.5,'Alamat',1);
	...
	$pdf->Cell(3,0.5,''.$data['nama_supplier'].'',1);
	$pdf->SetFont('Courier','B',7);
	$pdf->Cell(3,0.5,''.$data['phone'].'',1);
	$pdf->SetFont('Courier','B',7);
	$pdf->Cell(10.5,0.5,''.$data['alamat'].'',1);
	$pdf->Ln(0.5);
	}
